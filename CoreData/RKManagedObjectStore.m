...
- (id)initWithStoreFilename:(NSString *)storeFilename inDirectory:(NSString *)nilOrDirectoryPath usingSeedDatabaseName:(NSString *)nilOrNameOfSeedDatabaseInMainBundle managedObjectModel:(NSManagedObjectModel *)nilOrManagedObjectModel delegate:(id)delegate
{
    ...
    if (self) {
        ...
        if (nilOrManagedObjectModel == nil) {
            ...
            NSString *withoutExtension = [[storeFilename componentsSeparatedByString:@"."] objectAtIndex:0];
            NSString *modelPath = [[NSBundle mainBundle] pathForResource:withoutExtension ofType:@"momd"];
            NSURL *modelURL = [NSURL fileURLWithPath:modelPath];
            nilOrManagedObjectModel = [[NSManagedObjectModel alloc] initWithContentsOfURL:modelURL];
        }
        _managedObjectModel = nilOrManagedObjectModel;
